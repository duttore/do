  ...
  return (
    <div className="relative bg-black/5 backdrop-blur-sm rounded-2xl p-4 my-8">
      <div className="relative aspect-video rounded-xl overflow-hidden">
        ...
        <button onClick={handlePrev} className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white/20 p-2 rounded-full text-white">
          <FontAwesomeIcon icon={faChevronLeft} className="h-6 w-6" />
        </button>
        <button onClick={handleNext} className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white/20 p-2 rounded-full text-white">
        ...
        {mediaItems[currentIndex].type === 'video' && (
          <button
            onClick={togglePlay}
            className="absolute bottom-4 right-4 bg-white/20 p-2 rounded-full text-white"
          >
      ...
      <div className="mt-4 flex gap-2 overflow-x-auto pb-2">
        {mediaItems.map((item, index) => (
          <button
            ...
            className={`w-24 h-24 rounded-lg overflow-hidden ${currentIndex === index ? 'ring-2 ring-blue-500' : ''}`}
          >
...
export default function Home() {
  ...
  return (
    <div className="font-sans bg-gradient-to-b from-gray-50 to-gray-100 text-gray-900">
      <header className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-blue-600/80 backdrop-blur-md shadow-lg' : 'bg-transparent'}`}>
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="text-2xl md:text-3xl font-bold">
            <a href="#" className={`${scrolled ? 'text-white' : 'text-white'}`}>Stefano Ricci</a>
          </motion.div>
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className={`md:hidden p-2 rounded-lg ${scrolled ? 'text-white' : 'text-white'}`}>
            <FontAwesomeIcon icon={faBars} className="text-xl" />
          </button>
        </div>

        {menuOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="md:hidden absolute top-full left-0 right-0 bg-blue-600/80 shadow-lg rounded-b-lg">
            {[
              { href: '#servizi', icon: faWrench, text: 'Servizi' },
              { href: '#chi-sono', icon: faInfoCircle, text: 'Chi Sono' },
              { href: '#contatti', icon: faPhone, text: 'Contatti' }
            ].map((item, index) => (
              <a key={index} href={item.href} onClick={() => setMenuOpen(false)} className="flex items-center space-x-2 px-6 py-4 hover:bg-blue-700 text-white">
      ...
      <section className="relative min-h-screen flex items-center">
        <div className="absolute inset-0">
          <Image src="https://milanimacchinepercucire.com/wp-content/uploads/2024/02/milani_shop_desio-1024x575.webp" alt="Negozio Macchine da Cucire" layout="fill" objectFit="cover" priority className="brightness-50" />
        </div>
        <div className="container mx-auto px-4 relative z-10 pt-24">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-3xl mx-auto text-center text-white">
            <h1 className="text-4xl md:text-6xl font-bold leading-tight mb-6">Assistenza Professionale per Macchine da Cucire</h1>
            <p className="text-xl md:text-2xl mb-8 text-gray-200">Riparazioni rapide ed efficienti per garantire il massimo delle prestazioni della tua macchina da cucire.</p>
            <a href="#servizi" className="inline-flex items-center space-x-2 bg-yellow-500 text-white px-8 py-4 rounded-full hover:bg-yellow-600 transition-colors duration-300 shadow-lg">
      ...
      <section id="servizi" className="py-20">
        <div className="container mx-auto px-4">
          <motion.div {...fadeInUp} className="text-center mb-16">
            ...
            <p className="text-gray-600 max-w-2xl mx-auto">Offriamo soluzioni complete per la manutenzione e riparazione della tua macchina da cucire</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[{ icon: faWrench, title: 'Riparazioni', description: 'Riparazioni complete per ogni tipo di macchina da cucire, meccanica o elettronica.' },
              ...
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.2 }} viewport={{ once: true }} className="bg-white p-8 rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300">
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[{ text: "Servizio eccellente e riparazione veloce. La mia macchina da cucire funziona come nuova!", name: "Maria Rossi" },
              ...
              <motion.div key={index} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.2 }} viewport={{ once: true }} className="bg-gray-800 p-6 rounded-lg">
      ...
      <section id="contatti" className="py-20">
        <div className="container mx-auto px-4">
          <motion.div {...fadeInUp} className="text-center max-w-2xl mx-auto">
            ...
            <p className="text-gray-600 mb-8">Per richiedere informazioni o prenotare un appuntamento, puoi contattarmi anche su WhatsApp.</p>
            <div className="space-y-4">
              <a href="tel:+393343156903" className="flex items-center justify-center space-x-2 text-xl">
                <FontAwesomeIcon icon={faPhone} className="text-blue-600" />
                <span>+39 334 315 6903</span>
              </a>
              <a href="https://wa.me/393343156903" target="_blank" rel="noopener noreferrer" className="inline-flex items-center space-x-2 bg-green-500 text-white px-6 py-3 rounded-lg hover:bg-green-600 transition-colors duration-300">
